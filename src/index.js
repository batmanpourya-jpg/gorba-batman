...
async function api(request, env, path) {
  ...
  if (method==="POST" && path==="/api/profile") {
    const b=await request.json(), id=cleanId(b.id), name=cleanName(b.name);
    const r=await env.DIRECTORY.getByName("directory").updateProfile(id,name);
...
export class Directory extends DurableObject {
  constructor(ctx,env){
    ...
    this.sql.exec(`CREATE TABLE IF NOT EXISTS users(
      id TEXT PRIMARY KEY, name TEXT NOT NULL, avatar TEXT, created_at INTEGER NOT NULL
    )`);
  }
  async registerUser(id,name){
    ...
    return {ok:true,user:{id,name,avatar:""}};
  }
  async getUser(id){ return this.sql.exec(`SELECT id,name,avatar FROM users WHERE id=?1`,cleanId(id)).one() || null; }
  async searchUsers(q){
    const like=`%${cleanId(q)}%`;
    return this.sql.exec(`SELECT id,name,avatar FROM users WHERE id LIKE ?1 OR name LIKE ?1 ORDER BY id LIMIT 30`,like).toArray();
  }
...
export class UserInbox extends DurableObject {
  constructor(ctx,env){
    ...
    this.sql.exec(`CREATE TABLE IF NOT EXISTS chat_meta(
      chat_key TEXT PRIMARY KEY, peer_id TEXT NOT NULL, unread INTEGER NOT NULL DEFAULT 0,
  ...
  async saveMessage(m){
    ...
    this.sql.exec(`INSERT OR REPLACE INTO chat_meta(chat_key,peer_id,unread,last_body,last_time)
      VALUES(?1,?2,?3,?4,?5)`,m.chatKey,peer,unread,m.body,m.createdAt);
    if(m.direction==="in"){
  ...
  async listChats(){
    return this.sql.exec(`SELECT chat_key AS chatKey,peer_id AS peerId,unread,last_body AS lastBody,last_time AS lastTime
