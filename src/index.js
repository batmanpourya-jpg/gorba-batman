...
export class Directory extends DurableObject {
  ...
  allPeople(me) {
    const rows = this.ctx.storage.sql.exec(
      "SELECT id,name,avatar,cover,bio,created_at,last_seen,theme,primary_color,chat_background,chat_background_image,profile_show_bio,profile_show_last_seen,profile_show_avatar FROM users WHERE id<>? ORDER BY created_at ASC, name COLLATE NOCASE ASC LIMIT 200", me
    ).toArray();
