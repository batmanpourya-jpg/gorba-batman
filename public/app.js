...
async function openAdminPanel7(){
  ...
  try{
    ...
    $('#adminReports').innerHTML=(r.reports||[]).map(x=>`<div class="admin-report" data-report="${x.id}"><b>گزارش از: ${escapeHtml(x.user_id)}</b><small>هدف: ${escapeHtml(x.target_user||'مشخص نشده')}</small><p>${escapeHtml(x.message)}</p><div class="row"><select class="report-status"><option value="new"${x.status==='new'?' selected':''}>جدید</option><option value="in_progress"${x.status==='in_progress'?' selected':''}>در حال پیگیری</option><option value="closed"${x.status==='closed'?' selected':''}>بسته‌شده</option></select><input class="report-resolution input" placeholder="نتیجه پیگیری" value="${escapeHtml(x.resolution||'')}"><button class="primary report-save">ذخیره</button></div><small>${statusLabel[x.status]||'جدید'}</small></div>`).join('')||'<div class="empty-list">گزارشی نیست</div>';
...
async function loadChats(){if(!me)return;try{const d=await api('/api/conversations?id='+encodeURIComponent(me.id));$('#chats').innerHTML='';if(!d.conversations.length){$('#chats').innerHTML='<div class="empty-list">هنوز کسی وارد گوربا بتمن نشده.</div>';return}d.conversations.forEach(c=>$('#chats').append(chatItem(c))); try{await loadGroups()}catch(e){}}catch{$('#chats').innerHTML='<div class="empty-list">خطا در بارگذاری گفتگوها</div>'}}
function actionButtons(m){if(m.deleted)return null;const row=document.createElement('div');row.className='msg-actions';[['↩','پاسخ',()=>startReply(m)],['✎','ویرایش',()=>startEdit(m)],['📌',m.pinned?'برداشتن سنجاق':'سنجاق',()=>togglePin(m)],['⭐','ذخیره',()=>saveMessage(m)],['⌫','حذف',()=>deleteMessage(m)]].forEach(([txt,title,fn])=>{if((txt==='✎'||txt==='⌫')&&m.sender_id!==me.id)return;const b=document.createElement('button');b.textContent=txt;b.title=title;b.onclick=e=>{e.stopPropagation();fn()};row.append(b)});return row}
...
async async function sendReport(){const text=$('#reportText').value.trim();if(!text)return;try{await api('/api/report',{method:'POST',body:JSON.stringify({user_id:me.id,target_user:current?.id||'',message:text})});$('#reportText').value='';$('#reportErr').textContent='گزارش ارسال شد ✅'}catch(e){$('#reportErr').textContent=e.message}}
