...
function updateInfoPane(u){const pane=$('#infoPane');if(!pane||!u)return;pane.classList.remove('hidden');av($('#infoAvatar'),u);$('#infoName').textContent=u.name||'کاربر';$('#infoStatus').textContent=u.online?'● آنلاین':'● آفلاین';$('#infoStatus').style.color=u.online?'#20d27b':'#91a5bc';$('#infoBio').textContent=(u.profile_show_bio===0)?'بیوگرافی مخفی است':(u.bio||'بیوگرافی ثبت نشده');$('#infoLastSeen').textContent=u.online?'آنلاین':'آفلاین';try{const n=Number(u.message_count||0);$('#infoMsgCount').textContent=n||'—'}catch{}}
function hideInfoPane(){const p=$('#infoPane');if(p)p.classList.add('hidden')}
async function forwardMessage(m){
  if(!m||m.deleted||!me)return;
  const q=prompt('نام کاربری مقصد را وارد کن:');
  if(q===null)return;
  const name=q.trim();
  if(!name)return;
  try{
    const d=await api('/api/search?q='+encodeURIComponent(name)+'&id='+encodeURIComponent(me.id));
    const users=d.users||[];
    if(!users.length){alert('کاربری پیدا نشد');return;}
    let target=users[0];
    if(users.length>1){
      const list=users.map((u,i)=>(i+1)+'. '+u.name).join('\n');
      const pick=prompt('چند کاربر پیدا شد:\n'+list+'\nشماره را وارد کن:','1');
      const idx=Math.max(1,Number(pick||1))-1;
      target=users[idx]||users[0];
    }
    const body={sender_id:me.id,receiver_id:target.id,text:m.text||'',media_id:m.media_id||null,media_type:m.media_type||null,media_name:m.media_name||null,media_size:m.media_size||0,media_mime:m.media_mime||null};
...
bind('#login','click',login);bind('#name','keydown',e=>{if(e.key==='Enter')login()});bind('#send','click',send);bind('#text','keydown',e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send()}else sendTyping()});bind('#text','blur',()=>typing(false));bind('#logout','click',()=>{if(ws)try{ws.close()}catch{}clearInterval(presenceTimer);clearInterval(peopleTimer);localStorage.removeItem('gb_me');hideInfoPane();location.reload()});bind('#profileBtn','click',openProfile);bind('#closeProfile','click',()=>$('#profileBox')?.classList.add('hidden'));bind('#saveProfile','click',saveProfile);bind('#mobileOpen','click',()=>$('#side')?.classList.add('open'));bind('#mobileOpen2','click',()=>$('#side')?.classList.add('open'));bind('#mobileClose','click',()=>$('#side')?.classList.remove('open'));
...
bind('#pinnedBtn','click',openPinned);bind('#savedBtn','click',openSaved);bind('#infoProfileBtn','click',openProfile);bind('#infoChatSettingsBtn','click',openSettings);bind('#infoGalleryBtn','click',openGallery);bind('#infoPinnedBtn','click',openPinned);bind('#infoSavedBtn','click',openSaved);bind('#infoNotifyBtn','click',async()=>{try{if('Notification' in window){if(Notification.permission==='granted')alert('اعلان‌ها فعال هستند 🔔');else{const p=await Notification.requestPermission();alert(p==='granted'?'اعلان‌ها فعال شد ✅':'اعلان‌ها فعال نشد')}}}catch{}});enableNotifications();
