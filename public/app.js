...
async function sendReport(){const text=$('#reportText').value.trim();if(!text)return;try{await api('/api/report',{method:'POST',body:JSON.stringify({user_id:me.id,message:text})});$('#reportText').value='';$('#reportErr').textContent='گزارش ارسال شد ✅'}catch(e){$('#reportErr').textContent=e.message}}
