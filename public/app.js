...
function groupBubble(m){
  ...
  const tx=document.createElement('div');tx.textContent=m.text;d.append(tx);
...
async function loadChats(){if(!me)return;try{const d=await api('/api/conversations?id='+encodeURIComponent(me.id));$('#chats').innerHTML='';if(!d.conversations.length){$('#chats').innerHTML='<div class="empty-list">هنوز کسی وارد گوربا بتمن نشده.</div>';return}d.conversations.forEach(c=>$('#chats').append(chatItem(c))); await loadGroups();}catch{$('#chats').innerHTML='<div class="empty-list">خطا در بارگذاری گفتگوها</div>'}}
